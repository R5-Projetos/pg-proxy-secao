...
define('local_topico_ai_proxy/ui', ['jquery', 'core/ajax', 'core/notification', 'core/str'], function($, Ajax, Notification, Str) {
    ...
    console.log('[topico_ai_proxy] Module Loaded');
    console.log('[topico_ai_proxy] jQuery ($):', $);
    console.log('[topico_ai_proxy] Ajax:', Ajax);

    var AIButton = {
        init: function(courseId) {
            console.log('[topico_ai_proxy] Init called with courseId:', courseId);
            this.courseId = courseId;
            
            // Use delegated event listener to survive RemUI/Boost "More Menu" DOM manipulation
            if (!this.eventsAttached) {
                console.log('[topico_ai_proxy] Attaching delegated event listener');
                $(document).on('click', '.ai-generation-link, .ai-generation-btn', this.handleClick.bind(this));
        ...
        renderNavItem: function(container) {
            ...
            // Event is handled by delegation
            item.append(link);
        ...
        handleClick: function(e) {
            e.preventDefault();
            console.log('[topico_ai_proxy] Click detected!');
            var target = $(e.currentTarget);
            
            // Confirm dialog
            if (!confirm('Deseja realmente gerar a estrutura do curso com IA? Isso pode alterar tópicos existentes.')) {
                console.log('[topico_ai_proxy] Cancelled by user');
                return;
            }
            console.log('[topico_ai_proxy] Confirmed by user, starting generation...');
            ...
            .done(function(response) {
                console.log('[topico_ai_proxy] Response success:', response);
                Notification.alert('Sucesso!', 'A estrutura foi gerada. A página será recarregada.', 'OK', function() {
                     window.location.reload();
                });
            })
            .fail(function(ex) {
                console.error('[topico_ai_proxy] Response fail:', ex);
